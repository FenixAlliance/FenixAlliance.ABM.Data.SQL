...
SELECT c.cust_id, c.purchased_at, c.item
FROM customers as c
JOIN (
    ...
    ON c.cust_id = c1.cust_id and
      c.purchased_at = c1.min_purchased_at;
...
select distinct (substr(email, instr(email, '@'), 100)) 
from email;
...
SELECT distinct SUBSTRING(email, LOCATE('@', email) + 1) AS domain FROM email;
